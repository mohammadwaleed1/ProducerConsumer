...
void init() {       // initializing the mutex and sema
    ...
    sem_unlink("/full");
    sem_unlink("/empty");
    full = sem_open("/full", O_CREAT, 0644, 0);// sema full to used by waiter (value > 0) dish available (count = 0) no dishes
    empty = sem_open("/empty", O_CREAT, 0644, SIZE);//sema empty to used by chef (value > 0) table empty (count = 0) table full
}
